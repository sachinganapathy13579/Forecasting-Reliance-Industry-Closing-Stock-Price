...
st.title("Reliance Industry Closing Stock Price Forecasting")
...
def display_input_data():
    ...
    if reliance_stock_market_data is not None :

        st.success("Data Successfully loaded from 2015-01-01 to 2023-01-31")

        st.dataframe(reliance_stock_market_data,width=800)
    
    return reliance_stock_market_data
...
def main():
    ...
    if option == "EDA":
        ...
        reliance_stock_market_data=input_details
        ...
        if st.sidebar.checkbox("Select Close Feature to be predicted"):  

            st.write("## Select feature")  

            column_name=st.selectbox("",["Close"])
        ...
        if algorithm_name!="<select>":

            def get_classifier(name_of_classifer):

                if name_of_classifer == "LSTM" :

                    global scaler

                    closing_amount=input_details["Close"]

                    st.write("#### Closing Price of Reliance Stock from 2015-01-01 to 2023-01-31")
            ...
            if st.session_state.epochs:
                ...
                end_date_of_cuurent_data=datetime.datetime.strptime("2023-01-31","%Y-%m-%d")
                days=[]
